...
class FolderPath(SSHpathAbstract, _base.FolderPath):
	...
	def __init__(self, connection, remotePath):
		from _Walker import SSHFolderWalker
	...
	def _argsConvert_input(cls, funcName, argParser, args, toString):
		...
		return args
	def rmtree(self):
		return self.connection.runPyCode_singleFunction("shutil.rmtree(" + SSH.escapePath(self) + ")", ["shutil"])
...
def _buildFunc(funcName, argParser, pathInstance):
	...
	if isLocalFunc:
		...
		def __run(func, args):
			return pathInstance.connection.runPyCode_singleFunction(func + "(" + ", ".join(args) + ")", ["os","os.path"])
	return _run
